...
//require_once (dirname(__FILE__) . '/biostor/fetch.php');
...
require_once (dirname(__FILE__) . '/datacite/fetch.php');
...
require_once (dirname(__FILE__) . '/local/fetch.php');
require_once (dirname(__FILE__) . '/medra/fetch.php');
...
function classify_url($url)
{
	...
	// BioStor
	if (preg_match('/http[s]?:\/\/biostor.org\/reference\/(?<id>.*)$/', $url, $m))
	{
		$identifier = new stdclass;
		$identifier->namespace = 'BIOSTOR';
		$identifier->id = $m['id'];	
	}
	...
	// ORCID work
	if (preg_match('/https[s]?:\/\/orcid.org\/(?<orcid>([0-9]{4})(-[0-9A-Z]{4}){3})\/work\/(?<work>\d+)$/i', $url, $m))
	{
		$identifier = new stdclass;
		$identifier->namespace = 'ORCID_WORK';
		$identifier->id =  $m['orcid'] . '/work/' . $m['work'];
	}
...
function resolve_url($url)
{
	...
	{	
		...
		/*
			case 'BIOSTOR':
				$data = biostor_fetch($identifier->id);
				break;
		...
		*/
			case 'DOI':
				if (preg_match('/^(?<prefix>10\.\d+)\//', $identifier->id, $m))
				{
					switch ($m['prefix'])
					{
						// DataCite
						...
						// MEDRA
						case '10.12905':
							$data = medra_fetch($identifier->id);
							break;
		...
		/*		
			...
			*/
			...
			case 'ORCID_WORK':
				// Get an individual ORCID work
				$data = orcid_work_fetch($identifier->id);
				break;
...
if (0)
{
	...
	$url = 'https://doi.org/10.13128/Acta_Herpetol-13269';
